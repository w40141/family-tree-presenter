...
import type { FamilyTreeDataV1, Person, Relationship, UUID } from "./lib/domain/types";
...
function emptyTree(): FamilyTreeDataV1 {
  ...
  return {
    schemaVersion: "1.0",
    meta: { title: "Untitled", createdAt: t, updatedAt: t, app: { name: "family-tree", version: "0.1.0" } },
    people: {},
    relationships: {}
  };
}
...
function deletePersonCascade(data: FamilyTreeDataV1, personId: UUID): FamilyTreeDataV1 {
...
export default function App() {
  ...
  const peopleList = useMemo(() => {
    if (!data) return [];
    return Object.values(data.people).sort((p1, p2) => getPrimaryDisplayName(p1).localeCompare(getPrimaryDisplayName(p2), "ja"));
  }, [data]);
  ...
  function onExport() {
    ...
    const exportObj = {
      ...
      relationships: Object.values(data.relationships).sort((r1, r2) => r1.type.localeCompare(r2.type))
    };
  ...
  function onChangeDisplay(personId: UUID) {
    ...
    const names = p.names.map((n) => (n.primary ? { ...n, display: newDisplay } : n));
    setData({
      ...data,
      meta: { ...data.meta, updatedAt: nowIso() },
  ...
  return (
    <div style={{ padding: 16, maxWidth: 900, margin: "0 auto", fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>
        ...
        <button onClick={onClearAll} title="IndexedDBの保存も消します">Clear local storage</button>
      </div>
      ...
      <section style={{ border: "1px solid #ddd", borderRadius: 8, padding: 12, marginBottom: 12 }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
          <select value={coupleA} onChange={(e) => setCoupleA(e.target.value)} style={{ padding: 6 }}>
          ...
          <select value={coupleB} onChange={(e) => setCoupleB(e.target.value)} style={{ padding: 6 }}>
      ...
      <section style={{ border: "1px solid #ddd", borderRadius: 8, padding: 12 }}>
        ...
        {peopleList.length === 0 ? (
          <div style={{ opacity: 0.7 }}>まだ人物がいません。上で追加してください。</div>
        ) : (
                ...
                <div key={p.id} style={{ border: "1px solid #eee", borderRadius: 8, padding: 10 }}>
                  ...
                  <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                    <input
                      ...
                      onChange={(e) => setEditNameById({ ...editNameById, [p.id]: e.target.value })}
                      style={{ flex: 1, padding: 8 }}
                    />
                    <button onClick={() => onChangeDisplay(p.id)}>名前更新</button>
                  </div>
